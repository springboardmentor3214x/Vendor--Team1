...
def calculate_vendor_metrics(db: Session, vendor_id: int):
    ...
    if total_del > 0:
        ...
        delay_penalty = min(avg_delay * 3.0, 25)
        ...
        consistency_score = max(0, round(100 - std_dev * 8, 2))
...
def generate_vendor_rankings(db: Session):
    ...
    for v in vendors:
        ...
        rankings.append({
            ...
            "avg_service_rating": m["avg_service_rating"],
            "evaluated": bool(
                m["total_deliveries"] or m["total_messages"]
                or m["avg_quality_rating"] or m["avg_service_rating"]
            ),
        })
...
ON_TIME_STATUSES = ["Delivered On Time", "Delivered Early"]


def performance_dashboard(db: Session):
    from app.models.procurement import Procurement

    evaluated_vendor_ids = set()
    for model in (DeliveryPerformance, QualityEvaluation, CommunicationLog, ServiceRating):
        evaluated_vendor_ids.update(
            row[0] for row in db.query(model.vendor_id).distinct().all() if row[0] is not None
        )

    total_del = db.query(DeliveryPerformance).count()
    on_time = db.query(DeliveryPerformance).filter(
        DeliveryPerformance.delivery_status.in_(ON_TIME_STATUSES)
    ).count()
    delayed = total_del - on_time
    ...
    top_rankings = generate_vendor_rankings(db)[:5]

    return {
        "total_vendors_evaluated": len(evaluated_vendor_ids),
...
def get_vendor_performance_history(db: Session, vendor_id: int):
    ...
    proc_ids = {r.procurement_id for r in (*deliveries, *quality, *communications, *ratings)}
    procurements = {
        p.id: p for p in db.query(Procurement).filter(Procurement.id.in_(proc_ids)).all()
    } if proc_ids else {}
    purchase_orders = {
        po.procurement_id: po
        for po in db.query(PurchaseOrder).filter(PurchaseOrder.procurement_id.in_(proc_ids)).all()
    } if proc_ids else {}

    def context(procurement_id):
        proc = procurements.get(procurement_id)
        po = purchase_orders.get(procurement_id)
        return {
            "procurement_id": procurement_id,
            "request_number": proc.request_number if proc else None,
            "item_name": proc.item_name if proc else None,
    ...
    return {
        ...
        "quality_evaluations": [
            {
                "id": q.id,
                **context(q.procurement_id),
                "inspection_date": q.inspection_date,
                "material_quality": q.material_quality,
                "packaging_quality": q.packaging_quality,
                "quantity_accuracy": q.quantity_accuracy,
                "specification_compliance": q.specification_compliance,
        ...
        "communication_logs": [
            {
                "id": c.id,
                **context(c.procurement_id),
                "message_sent_time": c.message_sent_time,
                "vendor_response_time": c.vendor_response_time,
                "response_hours": c.response_duration_hours,
        ...
        "service_ratings": [
            {
                "id": s.id,
                **context(s.procurement_id),
                "professionalism": s.professionalism,
                "customer_support": s.customer_support,
                "documentation_quality": s.documentation_quality,
                "flexibility": s.flexibility,
                "communication_effectiveness": s.communication_effectiveness,
                "issue_resolution": s.issue_resolution,
